...
export interface BottomSheetProps extends Omit<HTMLAttributes<HTMLDivElement>, 'title'> {
  isOpen: boolean; close: () => void; unmount: () => void; closeOnBackdrop?: boolean;
  initialFocusRef?: RefObject<HTMLElement | null>; title: ReactNode; description?: ReactNode;
  footer?: ReactNode;
}
...
export function BottomSheet({ isOpen, close, unmount, closeOnBackdrop = true, initialFocusRef, title, description, children, footer, className, ...props }: BottomSheetProps) {
  ...
  return createPortal(
    <div className={cx('root', { open: isOpen, closing: !isOpen })} aria-hidden={!isOpen || undefined}>
      <div data-testid="bottom-sheet-backdrop" className={cx('backdrop')} onClick={(event) => {
        if (event.target === event.currentTarget && closeOnBackdrop && isOpen && isTopMost()) close();
      }}>
        <div {...props} ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby={titleId} aria-describedby={description ? descriptionId : undefined} tabIndex={-1} className={cx('panel', className)} data-testid="bottom-sheet-panel" onTransitionEnd={finishTransition}>
          <div className={cx('handle')} aria-hidden="true" />
